...
public:
    ...
    void update() {
        ...
        if (created) {
            ...
            for (int x = 0; x < GRID_SIZE; x++) {
                ...
                // Bonus fruit accent.
                if (fruits[x][y]->type < FRUITS_COUNT) continue;
                if (!fruits[x][y]->alive || fruits[x][y]->dropped || fruits[x][y]->selected) continue;
                ...
                if ((int)frand(5) == 0) {
                    ...
                    while (!goodPlace || n == GRID_SIZE * GRID_SIZE) {
                        ...
                        if (fruits[X][Y]->type < FRUITS_COUNT && !fruits[X][Y]->dropped && fruits[X][Y]->alive) {
                            ...
                            addAnimation("textures/ExtraFruitCreate.png", 64, 96, location, 26, 1.2f, 0.5f);
    ...
    // Move fruits to index position. Delete dead fruits and create new.
    void updateBoard() {
        ...
        if (fruits[X][Y]->type > FRUITS_COUNT) {
            ...
            for (int x = 0; x < GRID_SIZE; x++) {
                if (fruits[x][y]->selected && fruits[x][y] != fruits[X][Y]) {
                    // For cherry extra fruit.
                    float delay = 0.0f;
                    Vector2 prevIndex = fruits[x][y]->prevIndex;
                    if (fruits[x][y]->type == FRUITS_COUNT) { // swapped fruit is cherry
                        int fruitType = fruits[X][Y]->type;
                        for (int q = 0; q < GRID_SIZE; q++)
                        for (int p = 0; p < GRID_SIZE; p++) if (fruits[p][q]->type == fruitType || fruits[p][q] == fruits[x][y]) {
                            Vector2 location = getSkrewedLocation(x, y);
                            Tween* t1= TweenManager::getInstance()->addTween(fruits[p][q]->sprite, TweenType::SCALE_XY, 0.37f, Ease::Sinusoidal::InOut)
                                ->remove(true)->target(1.2f, 1.2f);
                            Tween* t2= TweenManager::getInstance()->addTween(fruits[p][q]->sprite, TweenType::SCALE_XY, 0.37f, Ease::Sinusoidal::InOut)
                                ->remove(true)->target(1.0f, 1.0f);
                            t1->addChain(t2)->start();
                            fruits[p][q]->kill(delay, FruitKillType::DEAD_EXTRA);
                            delay += 0.25f;
                        }
                        break;
                    } else if (fruits[X][Y]->type == FRUITS_COUNT) { // this fruit is cherry
                        int fruitType = fruits[x][y]->type;
                        for (int q = 0; q < GRID_SIZE; q++)
                        for (int p = 0; p < GRID_SIZE; p++) if (fruits[p][q]->type == fruitType || fruits[p][q] == fruits[X][Y]) {
                            Tween* t1= TweenManager::getInstance()->addTween(fruits[p][q]->sprite, TweenType::SCALE_XY, 0.37f, Ease::Sinusoidal::InOut)
    ...
    void onFruitKill(int x, int y, FruitKillType killType, float delay = 0.0f) {
        ...
        switch (killType) {
            ...
            // Dead with 'boom' animation.
            case FruitKillType::DEAD_EXTRA: {
                dyingFruits++;
                Vector2 location = getSkrewedLocation(x, y);
                Background* a = addAnimation("textures/ExtraFruitKill.png", 128, 128, location, 17, 0.7f, delay);
        ...
        switch(killType) {
            case FruitKillType::DEAD_EXTRA: {
                ...
                switch (fruits[x][y]->type) {
                    case 7: { // cherry: kill all fruits type with whom swapped
                    ...
                    case 10: { // chili: big bomb (3 x 3) kill by spiral
                        LOG_DEBUG("Kill chili!!!");
                        int d[12][2] = { // ta da!!! hard code...
