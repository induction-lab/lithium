...
public:
    ...
    Sprite* registerSprite(const char* texturePath, int32_t width, int32_t height) {
		...
		if (sprite->initialize() != STATUS_OK) return NULL;
    ...
    void draw() {
		...
		float timeStep = TimeManager::getInstance()->getElapsedTime();
		int32_t spriteCount = sprites.size();
