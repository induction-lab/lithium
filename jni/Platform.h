...
public:
	Platform():
		...
		// Base services.
		LOG_DEBUG("Creating base services:");
	...
	void run(ActivityHandler* activity) {
		...
		while (true) {
			...
			while ((result = ALooper_pollAll(isEnabledFlag ? 0 : -1, NULL, &events, (void**) &source)) >= 0) {
...
protected:
	...
	void activate() {
		...
		if ((!isEnabledFlag) && (application->window != NULL)) {
...
private:
	...
	static void callback_event(android_app* application, int32_t command) {
		Platform& eventLoop = *(Platform*) application->userData;
...
private:
	...
	void deactivateAccelerometer() {
		if (accelerometer != NULL) {
			LOG_DEBUG("Dectivating accelerometer.");
